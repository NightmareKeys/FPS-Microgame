...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    {
        //WallRun Function
        if (Input.GetKeyDown(wallRunKey) && !isGrounded)
        {
        ...
        IEnumerator endWallRun()
        {
            ...
            while (t < runtime)
			{
                ...
                if (isWallLeft)
				{
                    Physics.Raycast(transform.position, -transform.right, out hitLeft, wallrunDistance * 2);
                    if (hitLeft.distance > wallrunDistance)
                        t += runtime; //end our wallrun early

                }
                else //must be wall run right
				{
                    Physics.Raycast(transform.position, transform.right, out hitRight, wallrunDistance * 2);
                    if (hitRight.distance > wallrunDistance)
                        t += runtime; //end our wallrun early
                }
